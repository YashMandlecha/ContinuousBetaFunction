...
from setup import BetaFunctionException, SetupBetaFunction
...
class BetaFunction(SetupBetaFunction):
	...
	def _correlated_weighted_fit(self, x, y, fcn, weight_cov, prior=None, p0=None, extra_cov=None):
		...
		# Data-independent variance: residual prior uncertainty plus the
		# GP-discrepancy contribution, added as independent gvars so the
		# total error matches the full Bayesian posterior H^{-1} while the
		# data-dependent piece stays exactly correlated with the inputs.
		noise_cov = None
	...
	def _shifted_fit_beta(self, x_data, means, sdevs, times: list[str]):
		...
		The data are fit uncorrelated three times: central, all points shifted
		up by one standard deviation, and all points shifted down.  The quoted
		value is the central intercept with error equal to half the difference
		between the shifted intercepts.
		"""
		intercepts = []
		for sign in (1.0, -1.0):
	...
	def perturbative_interpolation(
		...
		Parametrizes beta(g^2) = beta_PT^(loops)(g^2) * (1 + sum_n c_n u^n)
		with u = g^2/(4 pi): the universal perturbative coefficients are fixed
		exactly, while the scheme/nonperturbative corrections c_n carry
		naturalness priors gvar(0, width).  By construction the fit ratio to
		perturbation theory goes to 1 as g^2 -> 0, injecting asymptotic
		freedom into the interpolation stage in a principled (prior-based)
		way instead of leaving the weak-coupling behavior to an unconstrained
		polynomial.
		"""
	...
	def _default_artifact_model(self) -> tuple[Callable, dict, dict]:
		...
		Symanzik effective theory with tree-level-normalized data implies the
		residual a^2/t artifacts enter at one loop and beyond, so the default
		slope is s(g^2) = u^2 (s0 + s1 u) with u = g^2/(4 pi) and naturalness
		priors on s0, s1 — the artifact vanishes like g^4 at weak coupling
		instead of being refit independently (and incoherently) at each g^2.
		"""
	...
	def cnt_xtrp_global(
		...
		The weight covariance defaults to the smooth product-kernel + nugget
		model over (log t, g^2) distances (cov_mode='kernel'), because the
		empirical covariance of an action-continuous dataset over the full
		plane is hopelessly singular.  gp=True adds a Kennedy-O'Hagan-style
		discrepancy kernel amp * x x' rho_t rho_g (vanishing at x = 0, so it
		cannot move the continuum intercept) with amplitude tuned by marginal
		likelihood.

		All fitted parameters — and therefore the stored continuum betas at
		every g^2 — are rebuilt as a linear map of the original data gvars,
		so the resulting curve is fully correlated point-to-point and with
		the upstream stages, as required for integrating it.
		"""
		...
		for flow, obs in flow_observable_pairs:
			...
			# Assemble the full (g^2, a^2/t) dataset for this flow/observable.
			...
			# Mean model and its prior/starting values.
			...
			# Continuum curve at x = 0, fully correlated across the grid.
			if intercepts == "free":
	...
	def interpolation_curve(self, flow: str, obs: str, flow_time: str, points: int = 200):
